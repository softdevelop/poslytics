	...
	public function addGroup() {
		if ($this->request -> isPost()) {
			...
			if ($this->UserGroup->addValidate()) {
				...
				$this->Session->setFlash('Nhóm người dùng được thêm thành công','flash_success');
	...
	public function editGroup($groupId=null) {
		if (!empty($groupId)) {
			if ($this->request -> isPut()) {
				...
				if ($this->UserGroup->addValidate()) {
					$this->UserGroup->save($this->request->data,false);
					$this->Session->setFlash('Nhóm người dùng được cập nhật thành công','flash_success');
	...
	public function deleteGroup($groupId = null) {
		if (!empty($groupId)) {
			if ($this->request -> isPost()) {
				...
				if($users) {
					$this->Session->setFlash('Xin lỗi một số người dùng được liên kết với nhóm này. Bạn không thể xóa!','flash_error');
					$this->redirect('/allGroups');
				}
				if ($this->UserGroup->delete($groupId, false)) {
					$this->Session->setFlash('Nhóm người dùng được xóa thành công','flash_success');
				}
			}
			$this->redirect('/allGroups');
		} else {
